...
def main(date) :
  ...
  file_list_in_bronze = s3_client.list_objects_v2(Bucket=bronze_bucket, Prefix='user/input/'+bronze_file_name+'/', Delimiter='/')
  try :
    if file_exists_check(file_list_in_bronze) :
      ...
      if silver_user_df.count() == 0 :
          ...
          file_list_in_silver = s3_client.list_objects_v2(Bucket=silver_bucket, Prefix='user/', Delimiter='/')
          file_list_in_silver_temp = s3_client.list_objects_v2(Bucket=silver_bucket, Prefix='user/temp/', Delimiter='/')
